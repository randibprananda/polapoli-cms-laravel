...
@section('title', 'Manage Activity | Pola Poli')
...
<div class="row mb-32 gy-32">
    <div class="col-12">
        <div class="row justify-content-between gy-32">
            <div class="col-12 col-md-4 d-flex">
                <a href="{{ route('dashboard.recent.index') }}" class="auth-back">
                    <i class="iconly-Light-ArrowLeft"></i>
                </a>
                <h4 class="mx-8 h4">Manage Activity</h4>
            </div>
    ...
    <div class="col-12">
        <div class="card hp-contact-card mb-32">
            <div class="card-body">
                <form action="{{route('dashboard.recent.store')}}" method="POST" id="formData" enctype="multipart/form-data">
                    @csrf
                    <div id="method">
                        @method("POST")
                    </div>
                            ...
                            <div class="mb-24">
                                ...
                                <div class="input-group">
                                    <span class="input-group-text slug"
                                        id="basic-addon3">https://polapoli.id/activity/</span>
                        ...
                        <div class="col-12 col-md-6">
                            <div class="mb-24">
                                ...
                                <select id="category_ids" class="category form-select" name="category_ids[]" multiple="multiple">
                                    @foreach ($recentAct_categories as $recentAct_category)
                                    <option value="{{$recentAct_category->id}}">{{$recentAct_category->title}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
<script>
    function setSelectedCategory(){
        ...
        // console.log("categioru "+arraySelectedCategory)
        if(selectedCategory != ""){
    ...
    function resetToSavedData() { 
        ...
        const id = $("#id").val()
        $.ajax({
                url: "{{route('dashboard.recent.show', ':id')}}".replace(":id", id),
                type: "GET",
    ...
    function updateData(){
        ...
        if(id != null && id != undefined){
            ...
            $("#formData").attr("action", "{{route('dashboard.recent.update', ':id')}}".replace(":id", id)); 
