...
@section('title', 'Add On | Pola Poli')
        ...
        <div class="col-12">
            <div class="row justify-content-between gy-32">
                <div class="col-12 col-md-4">
                    <h4 class="h4">Add On</h4>
                </div>
                <div class="col-12 col-md-7">
                    <div class="row g-16 align-items-center justify-content-end">
                        <div class="col-12 col-md-6 col-xl-4">
                            <form action="{{ route('dashboard.addOn.index') }}" method="get">
                            ...
                            <tbody>
                                @foreach ($data as $item)
                                    <tr>
                                        ...
                                        {{-- <td>
                                            Rp.
                                            {{ $item->title != null ? number_format($item->price ?? '-', 2, ',', '.') : 'Not set' }}
                                        </td> --}}
                                        <td>
                                            {{ $item->title != null ? $item->periode : 'Not set' }}
                                        </td>
                                        <td>
                                            @if ($item->status == 1)
                                                <div
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header py-16 px-24">
                    <h5 class="modal-title" id="modalDataLabel">Manage Add On</h5>
                ...
                <form id="formData" method="POST" action="{{ route('dashboard.addOn.store') }}">
                    @csrf
                    <div id="method">
                        @method('POST')
                    </div>
                        ...
                        <div class="row gx-8">
                            ...
                            <div class="col-12 col-md-12">
                                <div class="mb-24">
                                    ...
                                    {{-- <input type="number" class="form-control" name="periode" min="0" id="periode"
                                        required> --}}
                                    <select class="form-select" id="periode" name="periode" required>
                                        <option selected hidden>Periode</option>
...
@section('js')
    <script>
        ...
        function detailData(id = null) {
            if (id != null) {
                $.ajax({
                    url: "{{ route('dashboard.addOn.show', ':id') }}".replace(":id", id),
                    type: "GET",
        ...
        function updateData() {
            ...
            if (id != null && id != undefined) {
                ...
                $("#formData").attr("action", "{{ route('dashboard.addOn.update', ':id') }}".replace(":id", id));
        ...
        function deleteData() {
            ...
            if (id != null && id != undefined) {
                console.log("id = " + id);
                $("#formData").attr("method", "POST");
                $("#formData").attr("action", "{{ route('dashboard.addOn.destroy', ':id') }}".replace(":id", id));
